  ...
  // 채널 ID 추출 함수
  const extractChannelId = async (youtubelink) => {
    const usernameMatch = youtubelink.match(/@(.*)$/);
    if (usernameMatch) {
    ...
    const channelIdMatch = youtubelink.match(/channel\/([a-zA-Z0-9_-]+)/);
    return channelIdMatch ? channelIdMatch[1] : null;
  };

  // 특정 vtuber의 최신 동영상 가져오기
  app.get('/vtuber/:id/latest-videos', async (req, res) => {
    const { id } = req.params;

    // Check cache first
    const cacheKey = `latest-videos-${id}`;
    ...
    try {
      const connection = await oracledb.getConnection(dbConfig);
      const result = await connection.execute(
        'SELECT youtubelink FROM vtinfo WHERE id = :id',
        { id }
      );

      if (result.rows.length > 0) {
        const youtubelink = result.rows[0][0];
        const channelId = await extractChannelId(youtubelink);

        if (!channelId) {
          return res.status(400).json({ error: 'Invalid YouTube channel link' });
        }
      ...
      res.status(500).json({ error: 'Failed to fetch latest videos', details: err.message });
    }
  });

  // 특정 vtuber의 음악 재생목록에서 최신 동영상 3개 가져오기
  app.get('/vtuber/:id/music-videos', async (req, res) => {
    const { id } = req.params;

    const cacheKey = `music-videos-${id}`;
    const cachedData = cache.get(cacheKey);
    ...
    try {
      ...
      if (result.rows.length > 0) {
        ...
        const playlist = playlistsResponse.data.items.find(playlist =>
          ['COVER', 'Cover', '커버', '음악', '노래'].some(keyword => playlist.snippet.title.includes(keyword))
        );

        if (!playlist) {
          return res.status(404).json({ message: 'No music playlist found' });
        }
        ...
        const data = playlistItemsResponse.data.items;
      ...
      console.error('YouTube API error:', err.response ? err.response.data : err.message);
      res.status(500).json({ error: 'Failed to fetch music videos', details: err.message });
    }
  });
